...
requested_runs <- trimws(unlist(strsplit(
  Sys.getenv("ABC_CONV_RUNS", "rejection_core4,abcrf_core4,abcrf_all_variable"),
  ","
)))
requested_runs <- requested_runs[nzchar(requested_runs)]
valid_runs <- c("rejection_core4", "abcrf_core4", "abcrf_all_variable")
invalid_runs <- setdiff(requested_runs, valid_runs)
if (length(invalid_runs) > 0) {
  stop("ABC_CONV_RUNS contains invalid run names: ", paste(invalid_runs, collapse = ", "))
}
run_enabled <- function(run_name) run_name %in% requested_runs
...
cat("Convergence-test sim stats:", sim_stats_file, "\n")
...
cat("Convergence runs:", paste(requested_runs, collapse = ","), "\n")
...
sim_stats <- add_v2_derived_stats(fread(sim_stats_file))
...
common_ids <- intersect(sim_stats$ID, priors$ID)
...
for (rep_i in rep_index) {
  ...
  target <- copy(ref_table[ID == obs_id, setdiff(names(sim_stats), "ID"), with = FALSE])
  target[, ID := obs_id]
  true_dt <- priors[ID == obs_id]
  ref <- copy(ref_table[ID != obs_id])

  if (run_enabled("rejection_core4")) {
  ...
  for (stat_set in c(if (run_enabled("abcrf_core4")) "core4", if (run_enabled("abcrf_all_variable")) "all_variable")) {
