...
for (rep_i in seq_along(observed_ids)) {
  ...
  write_independent_outputs(summary_by_run, stat_manifest_by_run)
  fwrite(
    rbindlist(all_summaries, fill = TRUE),
    file.path(out_root, "posterior_summary_by_replicate.csv")
  )
  fwrite(
    rbindlist(stat_manifest),
    file.path(out_root, "summary_statistics_used_by_replicate.csv")
  )
}
...
write_independent_outputs(summary_by_run, stat_manifest_by_run)
accuracy <- accuracy_summary(summary_dt)
fwrite(accuracy, file.path(out_root, "convergence_accuracy_summary.csv"))

cat("Convergence test complete. Results saved in", out_root, "\n")
